import { Contest } from '../../models/contest';

export let contests = [{
  id: 1,
  name: 'WindOJ Test Round',
  description: 'Test round',
  enrollTime: new Date('2016/07/09 9:00').getTime(),
  startTime: new Date('2016/07/12 7:00').getTime(),
  endTime: new Date('2016/07/12 19:00').getTime(),
  isAvailable: true
},{
  id: 2,
  name: 'EFZ #1',
  description: 'Very easy round',
  enrollTime: new Date('2016/07/12 9:00').getTime(),
  startTime: new Date('2016/07/13 19:00').getTime(),
  endTime: new Date('2016/07/13 21:00').getTime(),
  isAvailable: true
}, {
  id: 3,
  name: 'EFZ #2',
  description: 'Very difficult round',
  enrollTime: new Date('2016/07/12 19:00').getTime(),
  startTime: new Date('2016/07/14 19:00').getTime(),
  endTime: new Date('2016/07/14 21:00').getTime(),
  isAvailable: true
}];
